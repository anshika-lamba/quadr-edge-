...
def evaluate_qadr():
    print("=" * 70)
    print("QADR Phase 4: Evaluating Accuracy Recovery (Validation Split N=950)")
    print("=" * 70)
    ...
    with open("data/manifest.csv", "r") as f:
        val_records = list(csv.DictReader(f))[50:]
    ...
    for idx, row in enumerate(val_records):
        ...
        for sig in severities:
            ...
            results[sig]['total'] += 1
            if pred_fp == oracle_label: results[sig]['fp32'] += 1
            if pred_int == oracle_label: results[sig]['int8'] += 1
            if pred_qadr == oracle_label: results[sig]['qadr'] += 1
            
        if (idx + 1) % 100 == 0 or (idx + 1) == len(val_records):
            print(f"[+] Evaluated {idx + 1}/{len(val_records)} validation scenes...")

    print("\n" + "=" * 75)
    print("           QADR Final Validation Metrics Summary (N=950)")
    print("=" * 75)
    print(f"{'Severity (σ)':<12} | {'FP32 Acc':<10} | {'INT8 Acc':<10} | {'QADR+INT8 Acc':<14} | {'Recovery Rate':<12}")
    print("-" * 75)
    
    md_lines = ["# QADR Final Validation Results Summary (N=950)\n",
    ...
    for sig in severities:
        ...
        denom = (fp_acc - int_acc)
        rec_rate = ((qadr_acc - int_acc) / denom * 100.0) if denom > 0 else 100.0
        rec_rate = max(0.0, min(100.0, rec_rate))
        
        print(f"{sig:<12.2f} | {fp_acc:<9.1f}% | {int_acc:<9.1f}% | {qadr_acc:<13.1f}% | {rec_rate:<11.1f}%")
        md_lines.append(f"| {sig:.2f} | {fp_acc:.1f}% | {int_acc:.1f}% | **{qadr_acc:.1f}%** | **{rec_rate:.1f}%** |")
        
    print("=" * 75 + "\n")
    
    with open("results/metrics_summary.md", "w") as f:
        f.write("\n".join(md_lines))
        
    print("[+] Final metrics summary saved to: results/metrics_summary.md")
